  ...
  (cl-who:with-html-output-to-string (s)
    (:html
     ...
     (:body
      ...
      (:div
       (let* ((number (hunchentoot:parameter "serial"))
	      (xor-number (format nil "~A" (logxor #x10FE5A (parse-integer number :radix 16)))))
	 (:h6 (format s "~A" (md5:md5sum-sequence xor-number)))))
      ;;(:h3 "App Database")
      ;;(:div
       ;;(:pre "SELECT version();"))
